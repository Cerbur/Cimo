...
| S2-T16 | 调整 `cimo.max-tool-rounds` 默认上限与 Agent Loop 工具轮次预算 | Ready | 默认 `cimo.max-tool-rounds` 收敛为 `128`；`<=0` fallback 到默认值；显式正数配置保持生效；达到上限时 `DefaultAgentLoop` 返回可诊断 `AgentEvent.Error`，不抛穿、不继续请求模型；补充配置绑定测试和 Agent Loop 上限测试；本阶段不引入 token 预算、用户确认续跑或复杂预算分层 |
