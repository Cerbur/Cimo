...
- 能运行本项目验证：至少支持用户确认后执行 `./gradlew test` 和 `./gradlew -p .cimo-workspace/step2-hello-rest test`；`./gradlew bootRun` 仍只作为人工 CLI 验收命令，不要求纳入 Agent 自动验收。
...
1. **环境准备**：用户确认或清理 `.cimo-workspace/step2-hello-rest`；代理检查目标目录状态，确认不会误覆盖主项目或旧验收产物。
2. **启动真实 CLI**：用户运行 `./gradlew bootRun` 并进入 `>` 提示符；代理根据用户反馈和必要的文件/配置检查确认 banner、CLI 输入和 `exit` / `quit` 基础行为仍可用。
3. **发起编码任务**：用户在 CLI 中要求 Cimo 创建 `.cimo-workspace/step2-hello-rest`，实现 `GET /hello` 返回 `{"code":200,"message":"success","data":"hello"}`；代理检查生成文件是否位于受控目录，且过程覆盖 `list` / `search` / `read` / `write` / `edit` 的实际能力。
4. **Bash 人工确认**：当 Cimo 请求执行 Bash 时，用户只在确认命令合理后输入精确 `y`；代理检查 BashTool 是否展示原始命令，未确认时是否稳定取消，确认后是否返回 exit code 和可用于修复的输出。
5. **测试验证**：用户让 Cimo 执行 `./gradlew -p .cimo-workspace/step2-hello-rest test`；代理检查测试是否断言 HTTP status `200`，以及 JSON 字段 `code`、`message`、`data` 分别为 `200`、`success`、`hello`。
6. **失败修复闭环**：若测试失败，用户把失败输出继续交给 Cimo；代理检查 Cimo 是否基于失败信息回到读 / 搜 / 改 / 验流程修复。失败未修复时不得标记 Done。
...
用户在人工验收第 3 步进入 Cimo CLI 的 `>` 提示符后，使用以下固定提示词发起 S2-T08 编码任务：

```text
请完成 Step 2 的 S2-T08 基本编写代码能力验收任务。

目标：在当前仓库的受控目录 `.cimo-workspace/step2-hello-rest` 下创建一个独立的最小 Spring Boot REST 项目，不要修改 Cimo 主项目结构。

要求：

1. 先使用 list / search 或等价工具检查 `.cimo-workspace/step2-hello-rest` 是否已经存在，避免误覆盖已有内容。
2. 如果目标目录不存在，请创建最小 Spring Boot 项目所需文件；如果已存在，请先读取关键文件并判断是否需要局部修改。
3. 项目需要提供 HTTP GET `/hello` 接口。
4. `/hello` 必须返回统一 JSON 响应：
...
5. 响应结构可以使用简单 DTO，例如 `ApiResponse<T>`，不要引入数据库、认证、前端、多环境配置或复杂全局响应框架。
6. 必须补充测试，使用 Spring MVC 测试或等价方式验证：
   - HTTP status 为 200
   - `$.code == 200`
   - `$.message == "success"`
   - `$.data == "hello"`
7. 文件创建和修改优先使用 write / edit 工具；需要查看文件时使用 read；需要定位文件或内容时使用 search。
8. 完成后请求执行验证命令：

./gradlew -p .cimo-workspace/step2-hello-rest test

9. 执行 Bash 前必须展示原始命令并等待我确认；只有我输入 y 才执行。
10. 如果测试失败，请根据失败输出继续读取、搜索、修改并重新验证；在测试通过前不要声称 S2-T08 已完成。
11. 最终请简要汇报：
    - 创建或修改了哪些关键文件
    - 使用了哪些工具能力
    - 验证命令和结果
