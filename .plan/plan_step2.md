...
### S2-16：Agent Loop 工具轮次预算

当前 `cimo.max-tool-rounds` 默认值为 `100000`，实际语义是单次用户输入中 `DefaultAgentLoop` 最多允许多少轮“assistant turn -> tool results -> 下一轮 assistant turn”的循环。它不是单个工具调用次数；一轮 assistant turn 内可能包含多个工具调用。

#### 第一性原理

这个配置解决的真实问题是：模型持续请求工具时，Agent Loop 必须有一个可预测的硬停止条件，避免因为模型循环、工具失败后反复重试或 provider 返回异常模式而无限请求模型和执行工具。同时，默认值不能小到误杀正常的读、搜、改、验、修闭环。

#### 设计约束

- 默认值从 `100000` 收敛为 `128`。`128` 对 Step 2 的小型编码任务已经足够宽松，同时比 `100000` 更像真实兜底，而不是近似无上限。
- `cimo.max-tool-rounds <= 0` 视为非法值，fallback 到默认值 `128`。
- 显式正数配置保持生效，不在 Step 2 做过大值 clamp；如果用户主动设置 `1000` 或 `100000`，表示用户明确扩大预算。
- 非数字配置继续交给 Spring Boot 配置绑定失败处理，不为了这一项引入额外字符串解析层。
- 达到上限时，`DefaultAgentLoop` 返回稳定 `AgentEvent.Error`，不继续请求模型、不抛穿 CLI；错误信息需要说明达到的上限，并提示任务可能在循环或需要调高 `cimo.max-tool-rounds`。
- Step 2 不引入 token 预算、工具调用总数预算、用户确认续跑、会话级预算继承或复杂预算分层；这些能力等 session/runtime 层成熟后再单独规划。

#### 候选实现方向

最小实现路径：

1. 在 `CimoProperties` 中提取默认常量，例如 `DEFAULT_MAX_TOOL_ROUNDS = 128`，避免配置、fallback 和测试各写各的数字。
2. 将 `application.yaml` 的 `cimo.max-tool-rounds` 默认值改为 `128`。
3. `CimoProperties` 保留 `<= 0` fallback，但 fallback 到默认常量。
4. 将 `DefaultAgentLoop` 的上限错误改为更可诊断的稳定文案，例如 `Reached max tool rounds: 128. The task may be looping or needs a higher cimo.max-tool-rounds value.`。
5. 补充配置绑定测试：默认值为 `128`、显式正数生效、`0` 和负数 fallback 到 `128`。
6. 补充 Agent Loop 上限测试：当模型持续请求工具并达到上限时，发出稳定 `AgentEvent.Error`，且不会额外继续请求模型。

### S2-17：Step 2 System Prompt 升级

当前 `CimoPrompts.STEP_1_SYSTEM_PROMPT` 仍描述 Step 1 能力：Cimo 是最小 CLI agent，唯一工具是只支持 `echo` 的 `bash`。这已经与 Step 2 的真实能力不匹配：工具集合、参数 schema、描述和能力边界已由 tool 层通过 `Tool` 暴露，并由 provider adapter 转换给模型；system prompt 继续声明 echo-only 会与 tool 层事实来源冲突。继续使用 Step 1 prompt 会让模型低估可用工具、误报“不支持”、或错误地把任务收缩到 echo。

#### 第一性原理

System Prompt 解决的真实问题不是替代工具安全边界，也不是重复声明工具 schema，而是给模型提供稳定的行为原则：知道自己是本地 CLI 开发 Agent，知道必须以运行时提供的 tool 声明为准，知道必须尊重工作区根目录、按先观察再修改再验证的方式行动，知道工具失败、用户取消或验证未执行时不能伪造成成功。

#### 设计约束

- Prompt 只描述当前 Step 2 的 Agent 行为原则，不提前承诺 Session、Web、REST API、MCP、网络搜索、多 Agent、完整权限系统或 OS sandbox。
- Prompt 必须集中维护在 `prompt` 包；入口层仍只引用 `CimoPrompts` 暴露的常量或工厂方法，不内联大段提示词。
- Tool 层是工具声明的事实来源：工具名、参数 schema、描述、职责和能力边界由 `Tool` 实现与 provider adapter 暴露；prompt 不列工具清单、不写工具参数、不声明单个工具职责，避免与 tool 层漂移。
- Prompt 可以要求模型以当前运行时提供的 tool 声明为准，并按“先观察、再最小修改、再验证”的原则选择工具；不能在 prompt 中硬编码 `read` / `list` / `search` / `write` / `edit` / `bash` 的能力说明。
- Prompt 应明确工作区边界：把当前工作目录当作工作区根目录；不要主动读取或修改工作区外路径；遇到工具拒绝、敏感文件、二进制文件、超限文件时根据错误信息调整方案，而不是绕过。
- Prompt 不声明 Bash 参数或 shell 细节；只说明本地命令执行可能需要用户确认，用户未确认、取消、工具返回失败或命令失败时，必须把结果当作真实失败/取消处理，不能声称命令已执行。
- Prompt 应明确输出风格：简洁说明计划、关键工具结果、已修改文件和验证状态；不编造文件内容、命令输出或测试结果；验证未执行时必须直说。
- Prompt 不负责实现或声明权限判断、路径校验、输出截断、工具能力或轮次预算；这些仍由 Tool、Agent Loop 和 runtime 代码承担。

#### 候选提示词结构

1. **身份与边界**：Cimo 是运行在本地受控工作区内的 CLI 开发 Agent；当前目标是协助读、搜、改、验代码。
2. **工具事实来源**：以运行时提供的 tool 声明、schema 和返回结果为准；prompt 不内联工具清单或参数。
3. **工作流原则**：先观察、再最小修改、再验证；遇到失败读取错误信息并迭代。
4. **安全与诚实**：尊重工作区边界、敏感/二进制/超限拒绝；不绕过工具错误；不伪造验证。
5. **用户沟通**：对用户保持简洁透明，说明做了什么、还缺什么、哪些命令需要用户确认。

#### 候选实现方向

最小实现路径：

1. 在 `CimoPrompts` 中新增 `STEP_2_SYSTEM_PROMPT`，保留 `STEP_1_SYSTEM_PROMPT` 作为历史常量，避免直接改名破坏引用。
2. 将 `CliAgentEntry` 的 `AgentContext.systemPrompt` 引用从 `STEP_1_SYSTEM_PROMPT` 切换到 `STEP_2_SYSTEM_PROMPT`。
3. 增加轻量测试或字符串约束测试，至少断言 Step 2 prompt 不再出现“only available tool is bash”“only supports echo through bash”等 Step 1 能力误导，并且不内联当前六个工具名形成工具清单。
4. 如需要，可补充一次真实 CLI smoke：让模型在 `.cimo-workspace` 中执行一个小的读 / 搜 / 写任务，确认不会再按 Step 1 echo-only 行为拒绝；不要求模型从 prompt 中复述工具清单。
...
7. 升级 Step 2 system prompt，移除 Step 1 echo-only 误导，让模型以 tool 层声明为准，并理解工作区边界、用户确认语义和诚实验证要求。
8. 最后执行 S2-T08 端到端编码闭环验收，并补充执行记录、验证结果和必要的真实 provider smoke 记录。
...
| S2-T16 | 调整 `cimo.max-tool-rounds` 默认上限与 Agent Loop 工具轮次预算 | Done | 默认 `cimo.max-tool-rounds` 收敛为 `128`；`<=0` fallback 到默认值；显式正数配置保持生效；达到上限时 `DefaultAgentLoop` 返回可诊断 `AgentEvent.Error`，不抛穿、不继续请求模型；补充配置绑定测试和 Agent Loop 上限测试；本阶段不引入 token 预算、用户确认续跑或复杂预算分层 |
| S2-T17 | 升级 Step 2 system prompt：移除 Step 1 echo-only 误导，切换为本地开发 Agent 行为引导 | Ready | `CimoPrompts` 新增 Step 2 system prompt 并由 CLI 入口引用；prompt 不声明工具清单、工具参数或单个工具职责，而是要求模型以运行时 tool 层声明和返回结果为准；prompt 明确工作区边界、用户确认语义、失败/取消处理和不伪造验证结果；不承诺未实现的 Session、MCP、Web、网络搜索或 OS sandbox；测试覆盖不再包含 Step 1 echo-only 误导，并确认 prompt 不把当前六个工具名内联成工具能力表 |
...
- 涉及 system prompt 时：
  - 验证 CLI 入口引用 Step 2 prompt，而不是 Step 1 echo-only prompt。
  - 验证 prompt 不内联工具清单、工具参数或单个工具职责；工具声明继续只来自 tool 层和 provider adapter。
  - 验证 prompt 说明工作区边界、用户确认和诚实验证要求，并要求模型以运行时 tool 声明和返回结果为准。
  - 验证 prompt 不包含“only available tool is bash”或“only supports echo through bash”等 Step 1 误导文案。
  - 默认用单元测试固定 prompt 关键约束；真实 provider smoke 可作为补充记录，不作为默认门禁。
- 不通过处理：
...
| Prompt 与真实能力漂移 | System prompt 仍描述 Step 1 echo-only，或重复声明工具清单后与 tool 层漂移 | Step 2 prompt 不声明工具清单、参数和单个工具职责；工具事实来源保持在 tool 层；用测试禁止 echo-only 误导和 prompt 内联工具能力表 |
...
| 2026-05-18 CST | S2-T16 收口为最小 Agent Loop 工具轮次预算：默认 `cimo.max-tool-rounds` 从 `100000` 收敛为 `128`；`<=0` fallback 到默认值；显式正数配置保持生效；达到上限时返回可诊断错误；本阶段不引入 token 预算、用户确认续跑或复杂预算分层。 | `100000` 更接近“近似无上限”，对模型循环缺少真实兜底；`128` 对 Step 2 读、搜、改、验、修的小型编码任务足够宽松，也能在异常循环时更早停止。token/确认续跑/预算分层需要 session/runtime 语义支撑，当前引入会扩大 Step 2 边界。 | 保持 `100000`；恢复 `5`；对显式大值做 clamp；现在引入 token 预算、工具调用总数预算或用户确认续跑。 | `CimoProperties`、`application.yaml`、`DefaultAgentLoop`、配置绑定测试、Agent Loop 上限测试、S2-T16 验收。 | 未提交 |
| 2026-05-18 CST | 新增 S2-T17：Step 2 system prompt 需要移除 Step 1 echo-only 误导，升级为本地开发 Agent 行为引导；工具清单、参数 schema、描述和职责继续由 tool 层声明，prompt 不重复声明。 | 当前 `CimoPrompts.STEP_1_SYSTEM_PROMPT` 与真实工具集合不匹配，会让模型低估能力、误拒绝任务或错误使用工具；但如果 prompt 反过来内联当前工具清单，又会与 `Tool` 事实来源重复并产生漂移风险。Prompt 应只约束工作区边界、用户确认、诚实验证和以运行时 tool 声明为准。 | 继续沿用 Step 1 prompt；在 prompt 中声明六个工具的职责和参数；直接删除 Step 1 prompt；把工具说明分散到入口层；在 prompt 中提前承诺 Session/MCP/Web/网络搜索等未来能力。 | `prompt` 包、`CliAgentEntry` 的 system prompt 引用、prompt 约束测试、真实 CLI smoke。 | 未提交 |
...
|------|---------|------|----------|------------|
| 2026-05-18 CST | S2-T16 | 将 `cimo.max-tool-rounds` 默认值从 `100000` 收敛为 `128`，在 `CimoProperties` 中提取 `DEFAULT_MAX_TOOL_ROUNDS` 作为配置默认与非法值 fallback 的单一来源；`application.yaml` 同步默认值。`DefaultAgentLoop` 达到工具轮次上限时返回更可诊断的 `AgentEvent.Error`，提示任务可能循环或需要调高 `cimo.max-tool-rounds`，不抛穿、不继续请求模型。补充 `CimoPropertiesTests` 覆盖默认值、显式正数、`0` 和负数 fallback；补充 `DefaultAgentLoopToolExecutionContextTests` 覆盖模型持续请求工具时达到上限会停止，并确认 client 调用次数等于预算值。JavaReview SubAgent 复查结论：通过，未发现 P0/P1/P2 问题。 | 首次 `JAVA_HOME=/opt/homebrew/opt/openjdk ./gradlew test --tests ai.cerbur.cimo.config.CimoPropertiesTests --tests ai.cerbur.cimo.agent.DefaultAgentLoopToolExecutionContextTests` 在沙箱内失败：Gradle wrapper lock 位于 `~/.gradle`，沙箱无权限；提权后同一定向测试通过。提权执行 `JAVA_HOME=/opt/homebrew/opt/openjdk ./gradlew test` 全量通过。 | 未提交 |
| 2026-05-18 CST | S2-T08 完成 | 人工启动真实 Cimo CLI，`cimo.work-dir` 收窄到 `.cimo-workspace`；被测 Cimo 按单行提示词创建 `.cimo-workspace/step2-hello-rest`，生成 `settings.gradle`、`build.gradle`、`HelloApplication`、`ApiResponse`、`HelloController` 和 `HelloControllerTest`。过程覆盖 `list`、`search`、`write`，首次 write 因父目录不存在失败后，经用户确认执行 `mkdir -p step2-hello-rest/src/main/java/example/hello step2-hello-rest/src/test/java/example/hello` 创建目录，再成功写入所有目标文件；未读取主项目 `.plan` / README / 源码，未生成 Maven `pom.xml` 或 Gradle wrapper。JavaReview SubAgent 复查结论：通过，未发现 P0/P1/P2 问题。 | 外层验证首次 `JAVA_HOME=/opt/homebrew/opt/openjdk ./gradlew -p .cimo-workspace/step2-hello-rest test` 因沙箱无法访问 Gradle wrapper lock 失败；提权后同命令通过，`BUILD SUCCESSFUL`，3 个 task executed。用户随后显式启动生成的 Spring Boot 应用后，提权执行 `curl -sS -i http://127.0.0.1:8080/hello` 验证真实 HTTP 请求，返回 `HTTP/1.1 200`，响应体为 `{"code":200,"message":"success","data":"hello"}`。 | 未提交 |
