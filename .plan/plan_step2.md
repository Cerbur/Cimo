...
|------|---------|------|----------|------------|
| 2026-05-14 CST | S2-T02 | 新增 `ToolExecutionContext(Path workingDirectory)`，将 `Tool.execute(...)` 签名收口为 `execute(ToolExecutionContext context, JsonNode arguments)`；`DefaultAgentLoop` 从 `AgentContext.workingDirectory` 构造窄上下文并传给 Tool；`BashTool` 使用该上下文设置进程工作目录；补充上下文归一化测试和 Agent Loop 传递上下文测试。 | `./gradlew test` 通过。 | 未提交 |
